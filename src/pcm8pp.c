...
int32_t pcm8pp_isavailable() {
  ...
  uint8_t version = _iocs_b_bpeek((uint8_t*)(eye_catch_addr + 7));
...
int32_t pcm8pp_get_mercury_version() {
  ...
  // exists?
  if (_dos_bus_err((uint8_t*)MERCURY_BOARD_CHECK_ADDR2, (uint8_t*)MERCURY_BOARD_CHECK_ADDR2, 2) == 0) {

    // 3.1 or 3.5/4.0
    uint8_t status = _iocs_b_bpeek((uint8_t*)MERCURY_STATUS_PORT);
    _iocs_b_bpoke((uint8_t*)MERCURY_COMMAND_PORT, status & 0b01111111);
    uint8_t status2 = _iocs_b_bpeek((uint8_t*)MERCURY_STATUS_PORT);
    rc = status2 & 0x80 ? 0x30 : 0x35;

    // 3.5 or 4.0
    if (rc == 0x35) {
      if (_dos_bus_err((uint8_t*)MERCURY_BOARD_CHECK_ADDR1, (uint8_t*)MERCURY_BOARD_CHECK_ADDR1, 2) != 0) {
        rc = 0x40;
      }
    }
  }
